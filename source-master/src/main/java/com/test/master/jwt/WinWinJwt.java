...
@Slf4j
@Data
public class WinWinJwt implements Serializable {
    ...
    public static WinWinJwt decode(String input){
        WinWinJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM,11);
            if(items.length >= 10){
                result = new WinWinJwt();
